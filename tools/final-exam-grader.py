...
final_exam = os.path.join(path, '..', 'final-exam.ipynb')
solutions = os.path.join(path, '..', 'solutions', 'final-exam.ipynb')
# Ensure data/ is available
os.chdir(os.path.join(path, '..'))

if not os.path.exists(final_exam):
    logger.critical('final-exam.ipynb was not found. You must have'
...
def checker(question_number, submissions, solutions):
    ...
    if question not in submissions:
        logger.error(f"Question {question_number} is WRONG: "
                     f"Variable {question} not found")
        return 0
    wrong = False
